...
function parseRows(raw: string): Parsed[] {
  ...
  // Clue exports often come as CSV; we also accept JSON arrays.
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
    try {
      ...
      return rows
        .map((r: any) => {
          const a = normalizeISO(String(r.periodStartDate ?? r.start ?? r.startDate ?? r.start_date ?? ''));
          const b = normalizeISO(String(r.periodEndDate ?? r.end ?? r.endDate ?? r.end_date ?? ''));
          if (!a || !b) return null;
  ...
  const startIdx = headers.findIndex((h) => ['start', 'start_date', 'period_start', 'startdate'].includes(h));
  const endIdx = headers.findIndex((h) => ['end', 'end_date', 'period_end', 'enddate'].includes(h));
...
export function inspectClue(raw: string, fileName: string): ImportInspection {
...
export function importClue(raw: string, fileName: string, selection: ImportSelection): ImportResult {
